...
THUMB_FUNC void LoadAllWazaTbl(struct WazaTbl * dest)
{
    ReadFromNarcMemberByIdPair(dest, NARC_POKETOOL_WAZA_WAZA_TBL, 0, 0, (NUM_MOVES + 1) * sizeof(struct WazaTbl));
}

THUMB_FUNC u32 GetWazaAttr(u16 waza, MoveAttr attr)
{
...
THUMB_FUNC u8 WazaGetMaxPp(u16 waza, u8 ppUp)
{
    u8 pp;
    if (ppUp > 3)
        ppUp = 3;
...
THUMB_FUNC u32 GetAttrFromWazaTbl(struct WazaTbl * wazaTbl, MoveAttr attr)
{
    switch (attr)
    {
...
THUMB_FUNC void LoadWazaEntry(u16 waza, struct WazaTbl * wazaTbl)
{
